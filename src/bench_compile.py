...
Reads docs/benchmarker_phase{1-6}*.json and prints:
  - Per-phase score table
  - Combined graded score
  - Notable findings (regressions, improvements)
  - Optionally writes docs/benchmarker_summary.json

Usage:
    python src/bench_compile.py              # print summary to stdout
    python src/bench_compile.py --save       # also write summary JSON
"""
...
def load_phase(phase_num):
    path = os.path.join(DOCS, PHASE_FILES[phase_num])
...
def main():
    ...
    parser.add_argument("--save", action="store_true",
                        help="Write summary to docs/benchmarker_summary.json")
    ...
    if args.save:
        out = os.path.join(DOCS, "benchmarker_summary.json")
