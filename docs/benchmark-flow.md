...
python src/bench_run_all.py

# Individual phases
python src/bench_phase1_explanation.py      # P1: Explanation tier (5 queries)
python src/bench_phase2_adversarial.py      # P2: Adversarial fabrication (5 queries)
python src/bench_phase3_workspace.py        # P3: Cross-workspace parity (5 queries)
python src/bench_phase4_stability.py        # P4: Depth stability (8 queries × 3 runs)
python src/bench_phase5_lmstudio.py         # P5: LM Studio validation (5 queries)
python src/bench_phase6_showcase.py         # P6: Showcase gallery (7 queries)

# Analysis
python src/bench_compile.py                 # Compile all results
python src/bench_compile.py --save          # Also save summary JSON

# Compare runs
python src/bench_diff.py old.json new.json
python src/bench_diff.py --before backup/ --after docs/
...
python src/bench_compile.py

# Save baseline results
mkdir -p results/baseline
cp docs/benchmarker_phase*.json results/baseline/
```
...
cp docs/benchmarker_phase*.json results/topN_8/
...
cp docs/benchmarker_phase*.json results/topN_20/

# Compare results
python src/bench_diff.py results/baseline/benchmarker_phase1_explanation.json \
...
python src/bench_phase1_explanation.py   # Test explanation tier changes
python src/bench_phase2_adversarial.py   # Verify fabrication guards still work

# Compare against baseline
python src/bench_diff.py results/baseline/benchmarker_phase1_explanation.json \
                          docs/benchmarker_phase1_explanation.json
```
...
python src/bench_phase3_workspace.py
...
python src/bench_phase4_stability.py
mkdir -p results/stability_run1
cp docs/benchmarker_phase4_stability.json results/stability_run1/

python src/bench_phase4_stability.py
mkdir -p results/stability_run2
cp docs/benchmarker_phase4_stability.json results/stability_run2/

# Compare stability between runs
python src/bench_diff.py results/stability_run1/benchmarker_phase4_stability.json \
...
python src/bench_run_all.py
cp docs/benchmarker_phase*.json results/pre_reindex/
...
# Diff all phases
python src/bench_diff.py --before results/pre_reindex/ --after docs/
```
...
3. **Skip Phase 1 for now:** Run other phases individually
   ```bash
   python src/bench_run_all.py 2 3 4 5 6
   ```
...
python src/bench_compile.py | grep -A 10 "Phase 2"
...
2. **Apply and retest:**
   ...
   python src/settings_manager.py set anythingllm -w c \
       --prompt prompts/anythingllm-system-prompt.md
   python src/bench_phase2_adversarial.py
   ```
...
cat docs/benchmarker_phase4_stability.json | jq '.analysis'
```
...
cp docs/benchmarker_phase*.json results/prompt_a/

# Test prompt B
python src/settings_manager.py set anythingllm -w c --prompt prompts/prompt_b.md
python src/bench_run_all.py
mkdir -p results/prompt_b
cp docs/benchmarker_phase*.json results/prompt_b/

# Compare all phases
python src/bench_diff.py --before results/prompt_a/ --after results/prompt_b/
...
cp docs/benchmarker_phase*.json results/continuous/$DATE/

# Compare against last run
LAST=$(ls -t results/continuous/ | head -2 | tail -1)
if [ -n "$LAST" ]; then
    python src/bench_diff.py \
...
python src/bench_phase6_showcase.py

# Extract S2 (price refusal) for README
cat docs/benchmarker_phase6_showcase.json | \
...
│   ├── bench_phase4_stability.py     # Phase 4 script (8 queries × 3 runs)
...
├── docs/
│   ├── benchmarker_phase1_explanation.json   # P1 results
│   ├── benchmarker_phase2_adversarial.json   # P2 results
│   ├── benchmarker_phase3_workspace.json     # P3 results
│   ├── benchmarker_phase4_stability.json     # P4 results
│   ├── benchmarker_phase5_lmstudio.json      # P5 results
│   ├── benchmarker_phase6_showcase.json      # P6 results
│   ├── benchmark-results-showcase.md         # Human-readable report
│   └── benchmark-flow.md                     # This file
├── prompts/
│   ├── anythingllm-system-prompt.md          # Workspace c/a prompt
│   ├── lm-studio-system-prompt.md            # LM Studio direct mode prompt
│   └── benchmarker.md                        # Benchmark specification
