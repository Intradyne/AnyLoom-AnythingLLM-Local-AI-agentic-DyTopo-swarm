...
→ rag_status()
  Observation: collection unhealthy or [N] points — confirms the failure is real
...
→ Desktop Commander: execute_command("curl -s http://localhost:6334/collections/lmstudio_docs")
  Observation: collection exists but [N] points — index was wiped
→ rag_reindex(force=true)
  Observation: re-indexed [N] files, [N] points total
→ rag_status()
  Observation: healthy, [N] points, [N] stale files
→ "The lmstudio_docs collection was empty — likely wiped by a container restart.
  ...
  Forced full reindex: now [N] points across [N] files.
  Per rag_status (before: [N] points, after: [N] points) and Desktop Commander (docker ps)."
```
...
→ swarm_start(task="Review for SQL injection, XSS, and auth bypass: [paste code]", domain="code")
  Observation: task_id "[task_id]" returned immediately
→ (wait 15-20 seconds)
→ swarm_status("[task_id]")
  Observation: round [round]/[max_rounds], Developer active, [elapsed]s elapsed
→ (wait 20 seconds)
→ swarm_status("[task_id]")
  Observation: round [round]/[max_rounds], Manager active, [elapsed]s elapsed
→ (wait 15 seconds)
→ swarm_status("[task_id]")
  Observation: completed, [N] rounds, [elapsed]s total
...
→ "DyTopo code swarm completed in [N] rounds ([elapsed]s):
  ...
  Per swarm_result on task [task_id]."
```
...
The Memory knowledge graph is a local, private, persistent store of stable facts that survives across sessions. It is one of the most valuable tools available — use it eagerly to recall context, record decisions, and build institutional knowledge over time. Core discipline: always `search_nodes` before creating entities. If found, use `add_observations` to append. Follow PascalCase entity names, snake_case types and relations.
...
Key principle: `search_nodes` → `add_observations` if exists → `create_entities` only if new → `create_relations` to link. Both agents on this machine access this graph — consistency matters.

<!-- Related: Memory, search_nodes, add_observations, create_entities, create_relations, search-before-create, PascalCase, snake_case, local knowledge graph -->
...
→ rag_status()
  Observation: lmstudio_docs healthy, [N] points, [N] stale files
→ search_nodes("recent")
  Example Memory recall: DyTopoSwarm tau=[value], AnythingLlmConfig temp=[value],
  last session resolved rag_search empty collection issue
→ Desktop Commander: execute_command("docker ps --format '{{.Names}} {{.Status}}'")
  Observation: anythingllm-qdrant Up [uptime], lmstudio-qdrant Up [uptime]
→ "Session oriented:
  - RAG index healthy: [N] points, [N] stale (per rag_status)
  - Memory recalls: [recalled settings and recent context]
...
→ Example response format: "[Asset] is trading at [tool-returned price] ([source_name](tool-returned-url))."
```
...
Correct flow: (1) recognize query involves time-sensitive data, (2) call Tavily BEFORE writing any answer text, (3) read the result, (4) build answer from tool result with inline source link. Do not generate an answer from training knowledge and then call a tool to verify — the initial answer anchors the response and the tool result gets ignored or fitted around it. Never fabricate a source name — the source name in the response must come from the tool result's source field, not from training knowledge. Presenting a training-era price attributed to an invented source name is the worst hallucination failure mode.

The HARD STOP applies regardless of framing — "ballpark", "approximately", "roughly", "based on training data", "last you knew", "even if it's not current", and "what was it worth when..." are all treated identically to "current price." Never output any numeric price, rate, or financial figure for any tradeable asset, regardless of how the user frames the request.

Key principle: Real-time data = Tavily first, one call, build answer from result. Every value (price, source name, URL) in the response comes exclusively from the tool result. Fetch is for known static URLs with clean HTML, not live data queries.

<!-- Related: live data, Tavily, real-time prices, scores, weather, news, financial data, one-call resolution, market data, HARD STOP, ballpark, approximately, roughly, price fabrication guard, broadened framing -->
...
→ Tavily("[asset] price per ounce") → [tool-returned price] from [source_name]
→ Example response format: "[Asset] is at [tool-returned price] ([source_name](tool-returned-url))." One call.
...
Key principle: Tavily → Fetch → Playwright. Financial sites return JS-heavy HTML that truncates via Fetch — call Tavily for market data. Cite source URLs inline. Every price, source name, and URL in the response must come from the tool result — never invent a source name or recite a price from training knowledge.
...
→ Filesystem: read_file("C:\Users\User\Qdrant-RAG+Agents\src\qdrant_mcp_server.py")
  (MCP Filesystem tool — reads, writes, searches, and lists files. NOT Windows Explorer or File Explorer.)
  Observation: line [line N]: RAG_CPU_THREADS = int(os.environ.get("RAG_CPU_THREADS", "8"))
  Also note: OMP_NUM_THREADS and MKL_NUM_THREADS set to same value at nearby lines
→ /think — changing CPU threads affects OMP and MKL too, should I update all three?
  ...
  Only need to change the RAG_CPU_THREADS line.
→ Filesystem: write_file — update line [line N]: "8" → "12"
→ Filesystem: read_file — confirmed line [line N] now reads "12"
→ "Updated RAG_CPU_THREADS default from 8 to 12 in qdrant_mcp_server.py line [line N].
...
## Response depth matches question complexity

<!-- Verified: 2026-02-14 -->

Match depth to complexity. When in doubt, default to the shorter tier — overshooting is a quality failure:

- **Lookup** (price, score, status, single fact, "what is X?", "what is [component]?"): Call the tool first, then 1-3 sentences built from the tool result with source link. Stop. Do NOT add ### headers, bullet lists, feature breakdowns, deployment details, comparisons, or a summary section. The system prompt includes concrete lookup examples (port 6333 lookup, DyTopo definition) each followed by "That is the complete answer" reinforcement.
- **Explanation** (how-to, comparison, "how does X work", "what's the difference between X and Y"): Two short paragraphs, 50-150 words maximum. NO ### headers, NO bullet lists, NO numbered lists, NO bold. Flowing prose only. The system prompt includes a concrete explanation example (80 words, two paragraphs, no formatting) with the reinforcement: "If you catch yourself writing a bullet point or header for an explanation query, STOP and rewrite as prose."
- **Complex task** (debugging, architecture, multi-step research): full structured response with evidence.

A user who asks "What port does Qdrant use?" wants `"Port 6334 for MCP hybrid search, port 6333 for AnythingLLM (per rag_search on 01-architecture-reference)."` — not a full architecture overview. End with the answer. Do not append "Let me know if you'd like..." or "Would you like me to..." offers.

<!-- Related: output proportionality, response depth, question complexity, concise answers, no follow-up offers, lookup tier, explanation tier, 50-150 words, depth calibration, That is the complete answer, what is X, no headers, no bullet lists, flowing prose -->
