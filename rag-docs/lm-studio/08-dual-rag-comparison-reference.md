...
<<<<<<< HEAD
The AnythingLLM pipeline on port 6333 uses a low document similarity threshold (the lowest available in workspace settings), retrieving up to 16 snippets per query with 30 messages of conversation history. This permissive threshold admits more candidate chunks at the cost of potential noise — a worthwhile trade-off given BGE-M3's embedding quality and the 80K context window that accommodates extra retrieved content. The context budget at 80K: system prompt ~2K, tool definitions ~3K, RAG snippets ~8K (16 × ~500 tokens), chat history ~12K, totaling ~25K overhead with ~55K remaining for the current exchange. Note: this ~8K RAG snippet budget is AnythingLLM's auto-injected context only (port 6333). LM Studio's explicit `rag_search` results (port 6334) come from its own separate ~71K remaining budget after ~8.9K system prompt and tool definition overhead. The two RAG budgets are independent — each pipeline has its own context allocation.
=======
The AnythingLLM pipeline on port 6333 uses a low document similarity threshold (the lowest available in workspace settings), retrieving up to 16 snippets per query with 30 messages of conversation history. This permissive threshold admits more candidate chunks at the cost of potential noise — a worthwhile trade-off given BGE-M3's embedding quality and the 80K context window that accommodates extra retrieved content. The context budget at 80K: system prompt ~2K, tool definitions ~3K, RAG snippets ~8K (16 × ~500 tokens), chat history ~12K, totaling ~25K overhead with ~55K remaining for the current exchange.
